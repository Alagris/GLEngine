...
namespace gle {
    ...
    namespace index {
        const void generateSubEBO(const aiMesh*const mesh,GLuint *const buffer,const unsigned int meshIndex,const ElementBufferObject&targetEBO,const unsigned int numIndicesPerFace) {
            copy(mesh,numIndicesPerFace,buffer);
        ...
        const void generateSubEBO(const aiMesh*const mesh,const unsigned int meshIndex,const ElementBufferObject&targetEBO,const unsigned int numIndicesPerFace) {
            const unsigned int size=targetEBO.getElementArraySizeInElements(meshIndex);
            if(size==0)return;
            std::unique_ptr<GLuint[]> bufPtr(new GLuint[size]);
            generateSubEBO(mesh,bufPtr.get(),meshIndex,targetEBO,numIndicesPerFace);
        }
    }
    ...
    namespace vertex {
        void generateSubVBO(const aiMesh*const mesh,GLfloat *const buffer,const VertexBufferObject & targetVbo) {
            ...
            glBufferSubData(GL_ARRAY_BUFFER,
                            ...
                            convertVboElementsNumberToBytesNumber(targetVbo.getVertexArraySizeInElements()),
                            static_cast<void*>(buffer));
        }
        void generateSubVBO(const aiMesh*const mesh,const VertexBufferObject & targetVbo) {
            const unsigned int size=targetVbo.getVertexArraySizeInElements();
            if(size==0)return;
            std::unique_ptr<GLfloat[]> bufPtr(new GLfloat[size]);
            generateSubVBO(mesh,bufPtr.get(),targetVbo);
        }
    }
    namespace color {
        void generateSubVBO(const aiMesh*const mesh,GLfloat *const buffer,const unsigned int colorSetIndex,const VertexBufferObject & targetVbo) {
            copy(mesh,colorSetIndex,buffer);
            glBufferSubData(GL_ARRAY_BUFFER,
                            getColorArrayOffsetInBytes(targetVbo,colorSetIndex),
                            convertVboElementsNumberToBytesNumber(targetVbo.getColorArraySizeInElements()),
                            static_cast<void*>(buffer));
        }
        void generateSubVBO(const aiMesh*const mesh,const unsigned int colorSetIndex,const VertexBufferObject & targetVbo) {
            const unsigned int size=targetVbo.getColorArraySizeInElements();
            if(size==0)return;
            std::unique_ptr<GLfloat[]> bufPtr(new GLfloat[size]);
            generateSubVBO(mesh,bufPtr.get(),colorSetIndex,targetVbo);
        }
    }
    namespace normal {
        void generateSubVBO(const aiMesh*const mesh,GLfloat *const buffer,const VertexBufferObject & targetVbo) {
            copy(mesh,buffer);
            glBufferSubData(GL_ARRAY_BUFFER,
                            targetVbo.getVertexNormalArrayOffsetInBytes(),
                            convertVboElementsNumberToBytesNumber(targetVbo.getVertexNormalArraySizeInElements()),
                            static_cast<void*>(buffer));
        }
        void generateSubVBO(const aiMesh*const mesh,const VertexBufferObject & targetVbo) {
            const unsigned int size=targetVbo.getVertexNormalArraySizeInElements();
            if(size==0)return;
            std::unique_ptr<GLfloat[]> bufPtr(new GLfloat[size]);
            generateSubVBO(mesh,bufPtr.get(),targetVbo);
        }
    ...
    namespace texture {
        void generateSubVBO(const aiMesh*const mesh,GLfloat *const buffer,const unsigned int textureSetIndex,const VertexBufferObject & targetVbo) {
            copy(mesh,textureSetIndex,buffer);
            glBufferSubData(GL_ARRAY_BUFFER,
                            getTextureArrayOffsetInBytes(targetVbo,textureSetIndex),
                            convertVboElementsNumberToBytesNumber(targetVbo.getTextureArraySizeInElements(textureSetIndex)),
                            static_cast<void*>(buffer));
        }
        void generateSubVBO(const aiMesh*const mesh,const unsigned int textureSetIndex,const VertexBufferObject & targetVbo) {
            const unsigned int size=targetVbo.getTextureArraySizeInElements(textureSetIndex);
            if(size==0)return;
            std::unique_ptr<GLfloat[]> bufPtr(new GLfloat[size]);
            generateSubVBO(mesh,bufPtr.get(),textureSetIndex,targetVbo);
        }
    }
...
//        copyVertexNormals(mesh,buffer);
//        glBufferSubData(GL_ARRAY_BUFFER,
//                        targetVbo.getVertexNormalArrayOffsetInBytes(),
//                        convertVertexNormalElementsNumberToBytesNumber(targetVbo.getVertexNormalArraySizeInElements()),
//                        static_cast<void*>(buffer));
//    }
//    namespace {
...
//        if(size==0)return;
//        std::unique_ptr<GLfloat[]> bufPtr(new GLfloat[size]);
//        generateVertexNormalsSubVBO(mesh,bufPtr.get(),targetVbo);
//    }
//
//
//    namespace {
